...
Chrome's WebMCP imperative API exposes twenty narrow tools for workspace inspection, parameter
...
For the jury path, the optional local Companion exposes twelve ChatGPT actions: four stateless
image tools and eight relayed commands for the visible Studio. The relay includes a two-step Magic
Wand flow, so ChatGPT first shows the black connected logo background and only removes it after an
explicit confirmation. No image bytes cross that relay.
