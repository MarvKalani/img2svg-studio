...
test("Given advanced tracing controls, when changed and reset, then live preview and canonical defaults remain accessible", async ({
  ...
  await page.goto("/");
  await expect(page.locator("body")).not.toContainText("VTracer");
  await expect(page.getByRole("heading", { name: "Vektorisierung" })).toBeVisible();
  ...
  const gradientStep = page.getByRole("slider", { name: "Vektorisierungs-Verlaufsschritt" });
  ...
  await expect(page.getByRole("combobox", { name: "Vektorisierungs-Kurvenmodus" })).toHaveValue(
    "spline",
  );
  await expect(page.locator("#path-precision")).toHaveValue("2");
});

test("Given the vectorization handbook, when contextual help is enabled or disabled, then hover explanations follow that state", async ({
  page,
}) => {
  await page.goto("/");
